> [!WARNING]
> 本文件是历史资料，状态为 historical-reference。仅作为旧需求、旧设计或迁移前实现证据，不代表当前 GF_X 项目事实。当前事实以用户最新口头确认、Assets/Game/Scripts、项目知识库（AI自行维护）/wiki/PROJECT_MAP.md、项目知识库（AI自行维护）/wiki/ACTIVE_CONTEXT.md 和 active openspec 为准。
# 13-AudioModule 模块详设
...
- `项目知识库（AI自行维护）/wiki/历史资料/GDD-v2/systems/14-音效与环境音.md`（系统设计依据）
- `项目知识库（AI自行维护）/wiki/历史资料/GDD-v2/modules/15-VFXModule.md`（并发削峰模式参考）
- `项目知识库（AI自行维护）/wiki/历史资料/GDD-v2/modules/16-BotControllerModule.md`（BotLODChangedEvent 来源）
