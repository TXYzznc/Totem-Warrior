...
| **12-core-ui-screens** | 原始需求：10 个 Form 视觉补齐 + 联调验证；**已被 13 扩展取代** | 方案已演进到 13 | — |
| **13-ui-screens-complete** | **活跃**：GDD v2.1 新增"玩家自纹身"UI 承载（SelfTattooForm）+ 原有 10 个 Form 完整视觉补齐 + 端到端无阻断联调。范围扩展：新增 SelfTattooForm（Tab 键唤出、部位选择、读条 UI）。决策依据：用户 grill-me 5 条共识（包含范围、5 阶段流程、完整可玩验收、仅 UI 缺口补齐、3-5 天紧凑周期）。 | Phase B 刚启动（proposal/design/tasks/requirements 已完成，等待 art-ui 提示词 → codex-image-gen 出图 → client-unity 制作） | art-ui / codex-image-gen / client-unity / qa-engineer |
...
*最后更新：2026-06-29（openspec 13-ui-screens-complete 启动：用户 grill-me 阶段 A 5 条共识挖透，涵盖范围/决策/边界/验收/约束）*
