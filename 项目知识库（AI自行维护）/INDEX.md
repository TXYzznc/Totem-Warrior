...
- **2.5D 正交俯角相机系统（CameraModule）** — 2026-07-02 — 把「3D 透视相机拍 2D 立牌」改造为「正交 + 55° 俯角」，GDD 定位「似 Hades 精致 2.5D」。新增 `CameraModule : IGameModule, ILateTickable`（零外部依赖，不用 Cinemachine）：LateUpdate SmoothDamp 平滑跟随 + 死区 + lookahead 预偏 + 固定全图 bbox 边界 clamp + `PlayShake` 震动整合（替代原 `VFXModule.SpawnCameraShake` 的失效父节点锚点方案）。配套 `BillboardSprite`（sprite 绕世界 X 轴固定旋转校正俯角压扁，因俯角固定为常量只设一次不进每帧）+ `DepthSortedSprite`（`sortingOrder = baseOffset + RoundToInt(-worldPos.z*100)` 动态深度排序解决角色互穿，shadow 联动写宿主 order-1，位置脏检查 0 GC）。框架增量：`GameTickDriver` 新增 `ILateTickable` 接口 + `LateUpdate()` 遍历（原框架只有 `ITickable.OnUpdate`，无 LateUpdate 转发）。**联调中额外发现并修复**：`SpawnerModule.Ground`（居中原点，玩家/49 actor 分布坐标系）与 `MapGenModule.MapGen_Ground`（150×150，中心 (75,75)）两套地面坐标不对齐导致的 L 形视觉分界，修复为 `SpawnerModule` ground scale 与 `MapSize` 对齐。**已知遗留（范围外）**：`EnemyModule` 与 `SpawnerModule` 两套敌人生成系统并存，待后续独立 change 整合。详见 [openspec/specs/camera-system/](../openspec/specs/camera-system/) + [archive/2026-07-02-25-camera-2p5d-system/](../openspec/changes/archive/2026-07-02-25-camera-2p5d-system/)
