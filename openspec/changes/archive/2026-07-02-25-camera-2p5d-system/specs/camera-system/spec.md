...
当前相机是 3D 透视 55° 俯角、硬编码在 `SpawnerModule.CreateScene()` 里、完全静止不跟随，配 2D SpriteRenderer 立牌角色，造成近大远小 + 边缘压缩 + 角色互穿。本规范定义把相机改造为正交俯角 2.5D 系统：自写 `CameraModule : IGameModule` 平滑跟随、边界 clamp、lookahead、billboard 校正、深度动态排序、震动整合。世界保持 3D（XZ 平面移动，Y=高度），不重构移动/战斗/地图/瞄准。
