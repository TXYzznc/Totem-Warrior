# Spec — player-attack-system

> 本能力（capability）冻结 9 种伤害源 + 3 个支柱（Pillar A/B/C）+ 起手 Build 三选 UI。
> 验收以可观测事件 / 数值口径 / 时序为准；视觉签名（粒子配色 / 字号 / 描边）留 #19 visual-polish。
...
- ❌ Bot AI 主动发起武器攻击（BotControllerModule 维持现状，不接 WeaponModule）
- ❌ 多人 PvP 互打伤害
- ❌ 中性元素之外的武器自带元素加成
- ❌ Trait 实际效果数值（占位 EffectType 字段，公式由 #21 / gd-system 后续平衡）
- ❌ 弓蓄力 UI 进度条（视觉留 #19）
- ❌ 召唤物 / 环境陷阱 / 反伤 / 吸血 / 易伤抗性矩阵 / 元素穿透
- ❌ 武器 prefab 真实动画（本期 WeaponPrefab 可为空 GameObject 占位）
