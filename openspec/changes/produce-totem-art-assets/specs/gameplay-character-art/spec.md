...
系统 SHALL 在新的非旧目录路径下管理 2D 角色资源：`Assets/Game/Sprite/Actors/<character_id>/<action>/<direction>/` 用于可动画角色的切分帧，`Assets/Game/Sprite/NPC/<npc_id>/` 用于静态世界 Sprite，`Assets/Game/Sprite/UI/CharacterSelectForm/Portraits/` 用于角色选择立绘。旧 `Assets/Game/Sprite/Character` 和 `Assets/Game/Sprite/Characters` MUST NOT 被重新使用。
