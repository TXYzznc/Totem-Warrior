...
- 更新角色美术契约，以当前 GF_X runtime asset catalog 和 `Assets/Game/Sprites/Actors/` 目录替代历史 `Assets/Resources/Sprite/Character/` 路径及 Player2/3 动画要求。
...
- 美术源文件和处理结果存放在 `openspec/changes/produce-totem-art-assets/art/`；最终导入物位于新的 `Assets/Game/Sprites/Actors/` 与 `NPC/` 目录，禁止恢复旧目录。
