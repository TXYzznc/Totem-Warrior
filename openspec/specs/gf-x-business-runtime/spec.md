...
## Purpose
TBD - created by archiving change gf-x-business-runtime-refactor. Update Purpose after archive.
## Requirements
...
- **GIVEN** legacy scenes have been removed from the workspace
...
- **AND** historical scenes and old editor tools SHALL NOT be restored into the workspace.
...
- **GIVEN** Business AI DataTables under `GameData/AIData/DataTables/Business/*.json`
- **WHEN** the migration manifest is generated or inspected
- **THEN** every table SHALL be listed with its current AI JSON path, C# type, primary key and runtime target path.
