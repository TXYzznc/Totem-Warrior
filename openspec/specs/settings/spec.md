...
## Purpose
TBD - created by archiving change 10-settings-form. Update Purpose after archive.
## Requirements
...
`SettingsModule` MUST 声明 `ModuleCategory = 1`（Service 层），`Dependencies = [AudioModule, SaveModule]`。`InitAsync` 内部 MUST 从 `SaveModule.Data.Settings` 读值并调 `ApplyAll`；MUST NOT 发任何事件（项目 InitAsync 约束）。
