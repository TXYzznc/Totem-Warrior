...
public static class EntityExtension
{
    ...
    /// <param name="fxName">特效prefab, 相对路径Game/Prefabs/Entity/</param>
    ...
    public static void PopScreenText(this EntityComponent eCom, string text, Vector3 startWorldPos, float popDistance, float duration = 1f, float fontSize = 4f, string entityPrefab = null)
    {
        if (string.IsNullOrEmpty(entityPrefab))
        {
            Log.Warning("PopScreenText requires an entity prefab in the clean workspace.");
            return;
        }
        ...
        eCom.ShowEntity<SampleEntity>(entityPrefab, Const.EntityGroup.Effect, effectParms);
    }
    /// <summary>
    /// 创建飘字效果
    /// </summary>
    ...
    public static void ShowPopText(this EntityComponent eCom, EntityParams eParams, string content, Vector3 endPos, float duration = 0.5f, float fontSize = 5f, string entityPrefab = null)
    {
        if (string.IsNullOrEmpty(entityPrefab))
        {
            Log.Warning("ShowPopText requires an entity prefab in the clean workspace.");
            return;
        }
        ...
        eCom.ShowEntity<SampleEntity>(entityPrefab, Const.EntityGroup.Effect, eParams);
    }
    /// <summary>
    /// 创建Entity
    /// </summary>
    /// <param name="eCom"></param>
    /// <param name="pfbName">预制体资源名(相对于Assets/Game/Prefabs/Entity目录)</param>
