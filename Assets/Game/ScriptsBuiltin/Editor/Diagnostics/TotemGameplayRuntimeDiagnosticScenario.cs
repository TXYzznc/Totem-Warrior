...
namespace UGF.EditorTools
{
    public sealed class TotemGameplayRuntimeDiagnosticScenario : GFDiagnosticScenarioBase
    {
        ...
        private static void CheckMapLayout(GFDiagnosticScenarioContext context)
        {
            ...
            context.Detail("map.pcg.algorithm", "WorldPlan; BspMaxDepth is not a PCG input.");
            context.AssertEqual(4, map.Rooms.Length, "map.roomCount");
            context.Assert(map.IsPcgGenerated, "Map layout must be generated by the PCG module.");
            context.Detail("map.pcg.profile", TotemMapService.CurrentPcgRuntimeProfile);
            context.AssertEqual(TotemMapService.ActivePcgMapWidth, map.PcgWidth, "map.pcg.width");
            context.AssertEqual(TotemMapService.ActivePcgMapHeight, map.PcgHeight, "map.pcg.height");
            context.Assert(map.PcgMapData != null, "Map layout must retain PCG data for runtime rendering.");
            context.Assert(map.PcgVisualCount > 0, "PCG map must expose placed visual objects.");
            context.Assert(map.PcgContentHash != 0UL, "PCG map must expose a stable non-zero content hash.");
            context.Detail("map.pcg.validation", map.PcgValidationSummary ?? string.Empty);
        ...
        private static void AssertEnemySpawnAnchorConsumers(GFDiagnosticScenarioContext context, TotemMapSnapshot map, TotemActorSpawnInfo[] roster)
        {
            ...
            context.Assert(maxDistance > 1.25f, "Enemy AI spawn positions should disperse away from tight anchor clusters.");
            AssertParticipantSpawnSeparation(context, roster, "map.anchor.consumer.participantMinDistance");
            ...
            for (int i = 0; i < payloads.Length; i++)
            {
                ...
                context.Detail($"map.anchor.consumer.enemySpawnGroup.{payloads[i]}", counts[anchorIndex]);
            }
        }
        ...
        private static void CheckMapRuntimeObjects(GFDiagnosticScenarioContext context)
        {
            ...
            try
            {
                ...
                context.Detail("map.runtime.pcgProfile", TotemMapService.CurrentPcgRuntimeProfile);
                ...
                context.Assert(snapshot.isPcgGenerated, "Map runtime should render the PCG-generated map.");
                context.Assert(snapshot.pcgContentHash != 0UL, "Map runtime should expose the PCG content hash.");
                int expectedPcgCells = Mathf.Max(0, (map.CurrentMap?.PcgWidth ?? 0) * (map.CurrentMap?.PcgHeight ?? 0));
                context.Detail("map.runtime.expectedPcgCells", expectedPcgCells);
                context.Assert(snapshot.pcgCellObjectCount >= expectedPcgCells, "Map runtime should place one ground tile per PCG cell.");
                context.Assert(snapshot.pcgVisualObjectCount > 0, "Map runtime should create PCG visual sprites.");
                context.AssertEqual(0, snapshot.pcgMissingSpriteCount, "map.runtime.pcgMissingSpriteCount");
                context.Assert(snapshot.rootChildCount >= 5, "Map runtime root should contain PCG tile root, visuals and boundary walls.");
                context.AssertEqual(snapshot.rootChildCount + 1, snapshot.spawnedObjectCount, "map.runtime.spawnedObjectCount");
        ...
        private static void AssertParticipantSpawnSeparation(GFDiagnosticScenarioContext context, TotemActorSpawnInfo[] roster, string detailName)
        {
            float minDistance = float.MaxValue;
            int participantCount = 0;
            for (int i = 0; roster != null && i < roster.Length; i++)
            {
                if (!TotemActorService.IsParticipantKind(roster[i].Kind))
                {
                    continue;
                }
                ...
                {
                    if (!TotemActorService.IsParticipantKind(roster[j].Kind))
                    {
                        continue;
                    }

                    minDistance = Mathf.Min(minDistance, FlatDistance(roster[i].Position, roster[j].Position));
                }
            }
